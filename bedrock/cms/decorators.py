...
def prefer_cms(view_func=None, fallback_ftl_files=None, fallback_lang_codes=None):
    ...
    def decorator(func):
        ...
        def wrapped_view(request, *args, **kwargs):
            ...
            #
            # 1) Un-mark THIS request as being for a CMS page, to avoid
            # lib.l10n_utils.render() incorrectly looking for available
            # translations based on CMS data.
            request.is_cms_page = False

            # 2) Make extra sure THIS request is still annotated with any
            # CMS-backed locale versions that are available, so that we can
            # populate the language picker appropriately.
